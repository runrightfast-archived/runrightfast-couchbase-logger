...
(function() {
		...
		if (!this.cb) {
			this.cb = new Couchbase.Connection(this.options.couchbase, function(error) {
				setImmediate(function() {
					if (error) {
						self.emitAsync(COUCHBASE_LOGGER_EVENT.CONN_ERR, error, self);
						if (callback) {
							callback(error, self);
						}
					} else {
						self.emitAsync(COUCHBASE_LOGGER_EVENT.STARTED, self);
						if (callback) {
							callback(self);
						}
					}
				});
				self.emitAsync(COUCHBASE_LOGGER_EVENT.STARTING);
			});
		}
	};
	...
	CouchbaseLogger.prototype.stop = function() {
		if (this.cb) {
			this.cb.shutdown();
			this.emitAsync(COUCHBASE_LOGGER_EVENT.STOPPED, this);
			this.cb = undefined;
		}
	};

	CouchbaseLogger.prototype.emitAsync = function() {
		var self = this;
		var args = arguments;
		setImmediate(function() {
			self.emit.apply(self, Array.prototype.slice.call(args));
		});
	};
	...
	CouchbaseLogger.prototype.logEvent = function(event) {
		if (event) {
			...
			this.cb.set(event.uuid, event, function(error, result) {
				if (error) {
					self.emitAsync(COUCHBASE_LOGGER_EVENT.LOG_EVENT_ERR, error, event, self);
				} else {
					self.emitAsync(COUCHBASE_LOGGER_EVENT.LOGGED_EVENT, result);
				}
			});
		}
	};
